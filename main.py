...
def exercise_4(start_station, end_station, adjacency_List: dict = neighbour_stations()):
    def solve( graph, node1, node2):                                            
        path_list = [[node1]]                                                   # This keeps track of all paths explored
        path_index = 0                                                          # Pointer to keep track of what path is being explored in the instance
        previous_nodes = {node1}                                                # Keeps track of all the nodes that have been explored in the current path
        if node1 == node2:                                                      # Checking if the start node = end node
            return path_list[0]
            
        while path_index < len(path_list):                                      # Going through all the path in the path list
            current_path = path_list[path_index]                                # Obtaining the current path we are exploring
            last_node = current_path[-1]                                        
            next_nodes = graph[last_node]                                       # Obtaining the next nodes to visit from the adjacency list
            if node2 in next_nodes:                                             # Checking if the end node is in the list of next nodes
                current_path.append(node2)                                      # If it is, append to the list of current nodes
                return current_path                                             # Return a list of the current path, since we have reached destination
            for next_node in next_nodes:                                        # Exploring the next nodes that have not yet been explored
                if not next_node in previous_nodes:                             # Checking to ensure we have not yet visited that node, to avoid an endless loop or backtracking
                    new_path = current_path[:]                                  # Creating new path to keep track of our current path     
                    new_path.append(next_node)                                  # Appending the next node to the new path
                    path_list.append(new_path)                                  # Appending the new path to our path list
                    previous_nodes.add(next_node)                               # Adding the next node we visited to our set of previously visited nodes
        
            path_index += 1                                                     # Updating path index
        return []                                                               # Returns an empty list if no path is found after all the paths available have been explored

    return solve(adjacency_List, start_station, end_station)                    # Returns a list with the nearest stations
...
######################################################################################################################################################
# INITIAL ATTEMPTED SOLUTION USING OOP
